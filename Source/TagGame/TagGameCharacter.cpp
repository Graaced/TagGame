...
void ATagGameCharacter::GrabCube()
{

	Cube->AttachToActor(this, FAttachmentTransformRules::KeepRelativeTransform);
	Cube->SetActorRelativeLocation(FVector(0, 0, 0));

	bHasCube = true;

	// timer per rilasciare il cubo dopo 5 secondi
	GetWorld()->GetTimerManager().SetTimer(TimerHandle_DropCube, this, &ATagGameCharacter::DropCube, 5.0f, false);
	
	
}


void ATagGameCharacter::DropCube()
{
	if (Cube)
	{
		Cube->DetachFromActor(FDetachmentTransformRules::KeepWorldTransform);
		bHasCube = false;

		// clear del timer
		GetWorld()->GetTimerManager().ClearTimer(TimerHandle_DropCube);
	}
}
...
// TOGLI IL TIMER E FAI SOLO CHE SE PRENDO LA PALLA GLI ALTRI SCAPPANO; 
// LA IA NON DEVE VEDERE LA PALLA